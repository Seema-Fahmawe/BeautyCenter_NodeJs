...
import { generateToken, verifyToken } from '../../../service/generateAndVerifyToken.js';
...
export const confirmEmail = asyncHandler(async (req, res, next) => {
  ...
  const decoded = verifyToken(token, process.env.SIGNATURE_SIGNUP);
  if (!decoded?.email) {
    return next(new Error(`invalid token payload`, { cause: 400 }));
  }
  const user = await userModel.updateOne({ email: decoded.email }, { confirmEmail: true });
  const owner = await ownerModel.updateOne({ email: decoded.email }, { confirmEmail: true });
  if (user?.modifiedCount || owner?.modifiedCount) {
...
export const newConfirmEmail = asyncHandler(async (req, res, next) => {
  ...
  const { email } = verifyToken(token, process.env.SIGNATURE_SIGNUP);
  if (!email) {
    return next(new Error(`invalid token payload`, { cause: 400 }));
  }
  ...
  token = generateToken({ email }, process.env.SIGNATURE_SIGNUP, 60 * 5);
  const link = `${req.protocol}://${req.headers.host}/auth/confirmEmail/${token}`;
  const html = `<!DOCTYPE html>
     <html>
     <head>
...
export const signin = asyncHandler(async (req, res, next) => {
  ...
  if (user) {
    ...
    const token = generateToken({ id: user._id, role: user.role }, process.env.SIGNATURE_SIGNIN, '1h');
    const refresh_token = generateToken({ id: user._id, role: user.role }, process.env.SIGNATURE_SIGNIN, 60 * 60 * 24 * 365);
    return res.status(200).json({ message: "success", token, refresh_token });
  }
  if (owner) {
    const matchOwnerPassword = compare(password, owner.password);
    if (!matchOwnerPassword) {
      return next(new Error("invalid login data", { cause: 400 }));
    }
    const token = generateToken({ id: owner._id, role: owner.role }, process.env.SIGNATURE_SIGNIN, '1h');
    const refresh_token = generateToken({ id: owner._id, role: owner.role }, process.env.SIGNATURE_SIGNIN, 60 * 60 * 24 * 365);
    return res.status(200).json({ message: "success", token, refresh_token });
  }
})
...
export const sendCode = asyncHandler(async (req, res, next) => {
  ...
  let code = customAlphabet('123456789', 5);
  code = code();
  user = await userModel.findOneAndUpdate({ email }, { forgetCode: code }, { new: true });
  owner = await ownerModel.findOneAndUpdate({ email }, { forgetCode: code }, { new: true });
...
export const forgetPassword = asyncHandler(async (req, res, next) => {
  ...
  if ((code != user?.forgetCode || !code) && (code != owner?.forgetCode || !code)) {
  ...
  if (owner) {
    owner.password = hash(password);
    owner.forgetCode = null;
    owner.changePasswordTime = Date.now();
    await owner.save();
    return res.status(200).json({ message: 'success', owner });
  }
